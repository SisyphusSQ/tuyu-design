...
export const workflowDiagrams = new Map<string, DiagramSpec>([
  ...
  [
    ...
    diagram(
      ...
      'flow',
      '本地项目创建流程',
      '这张图回答：用户创建项目后，哪些项目清单、锁和健康状态会落地？',
      ...
      [
        ...
        item('写入项目清单', '单一清单 + 内部分区', 'source'),
        item('建立保存锁', '防止并发写入', 'app'),
        item('健康检查', 'schema、路径、权限', 'risk'),
      ],
      ['目录不可写时拒绝创建。', '项目清单版本不支持时只读打开或迁移。', '崩溃恢复不覆盖上一版有效文件。'],
    ),
  ],
  [
    'asset-library',
  ...
  [
    ...
    diagram(
      ...
      [
        item('参考输入组', '视频 / 图片 / 文本引用', 'source'),
        item('生产组', '任务意图 / 输出目标', 'app'),
        item('输出节点', '文本 / 图片 / 视频版本', 'flow'),
        item('运行历史', '当前 / 收藏 / 最近成功', 'risk'),
      ],
      ['非法关系不写入图谱。', '运行历史不作为图谱端点。', '缺能力时降级任务而不删除生产组。'],
    ),
  ],
  [
    'shot-prompt',
...
export const moduleDiagrams = new Map<string, DiagramSpec>([
  ...
  [
    ...
    diagram(
      ...
      [
        item('GraphNode', '只保存引用标识', 'source'),
        item('生产组', '外层可运行上下文', 'app'),
        item('参考组', '内层输入集合', 'flow'),
        item('运行历史', '运行历史入口', 'risk'),
      ],
      ['图谱不保存完整领域真相。', '生产组不替代镜头、提示词或资产。', '历史轨只展示关键成功版本。'],
    ),
  ],
  [
    'asset-continuity',
  ...
  [
    ...
    diagram(
      ...
      [
        item('Top Bar', '项目 / 保存 / 队列', 'source'),
        item('Left Panel', '资产 / 剧本 / 搜索', 'app'),
      ...
      [
        ...
        lane('反馈', [item('队列', '运行任务', 'app'), item('底部栏', '健康和错误', 'risk')], 'risk'),
      ],
    ),
  ],
  [
